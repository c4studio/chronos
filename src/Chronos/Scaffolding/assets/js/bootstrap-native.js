...
(function (root, factory) {
  ...
  var Affix = function(element,options) {
    ...
    this.pinTop = function () {
      if ( !/\baffix/.test(this.element.className) ) {
        ...
        this.affixed = true;
        var parentStyle = window.getComputedStyle(this.element.parentElement);
        this.element.style.width = (this.element.parentElement.offsetWidth - parseFloat(parentStyle.paddingLeft) - parseFloat(parentStyle.paddingRight)) + 'px'
      }
    }
  ...
  var Modal = function(element, options) { // element is the is the modal
    ...
    this.open = function() {
      var show = null, shown = null;

      //register events
      if (('CustomEvent' in window) && window.dispatchEvent) {
        show =  new CustomEvent("show.bs.modal");
        shown = new CustomEvent("shown.bs.modal");
      }
      if (show) { this.modal.dispatchEvent(show); }
      ...
      this.timer = setTimeout( function() {
        ...
        if (shown) { self.modal.dispatchEvent(shown); }
      }, this.options.duration/2);
    ...
    this.close = function() {
      //register events
      if (('CustomEvent' in window) && window.dispatchEvent) {
        hide =  new CustomEvent("hide.bs.modal");
        hidden = new CustomEvent("hidden.bs.modal");
      }
      if (hide) { this.modal.dispatchEvent(hide); }
      ...
      if (hidden) { this.modal.dispatchEvent(hidden); }
    }
    ...
    this.dismiss = function() {
      function dismissHandler(e) {
        if ( e.target.getAttribute('data-dismiss') === 'modal' || (e.target.parentNode && e.target.parentNode.getAttribute('data-dismiss') === 'modal') || e.target === self.modal ) {
  ...
  (function () {
    // TOOLTIP DATA API
    // =================
    var Tooltips = document.querySelectorAll('[data-tooltip=tooltip]'), i = 0, tpl = Tooltips.length;
    for (i;i<tpl;i++){
